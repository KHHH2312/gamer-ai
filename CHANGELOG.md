...
- SC2: `new_action_unlock_bonus` reward config key (issue #360). Fires a
  one-shot bonus per episode on the first *executable availability* of a
  tech-tree-gated action (e.g. `Build_Barracks_screen` when a `SupplyDepot`
  is completed, resources are affordable, and the right unit is selected).
  Only actions with at least one `required_buildings` precondition in the
  tech tree are eligible; selection-only and always-available actions are
  excluded. Default `0.0` — opt-in. Set in `reward_config.yaml`.
